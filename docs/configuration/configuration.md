...
Some of fields have not been enabled because they've not been implemented yet. Invalid fields are tagged *TBI* (To be implemented) in this document.
...
| liferay Version | This is taken from the field of ```-v``` in ```damascus -init``` command. ```70``` means Liferay 7.0 / DXP. The default as of 2017 June is ```70```. This will be always the latest version in future when a new version of Liferay is released. |
...
| customValue | Customize values by array that you want to use in templates. |
...
| name | This field name. This will be also used a field name for ```service.xml```  |
...
### Boolean
This will generate boolean field
...
| categories | This generates category field in Edit page. Default value is `false`.
| discussion | This generates comment section in Edit page. Default value is `false`.
| ratings | This generates rating section in Edit page. Default value is `false`.
| tags | This generates tags section in Edit page. Default value is `false`.
| relatedAssets | This generates related assets section in Edit page. Default value is `false`.
...
| generateActivity | This field enables activity for the activity portlet. Default value is `false`.
